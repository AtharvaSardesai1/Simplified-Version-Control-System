...
void deleteMostRecentCommit(repository* repo, commitStack* stack) {
    ...
    for (int i = 0; i < N; i++) {
        if (repo->nodes[i] == NULL) continue; // Skip if branch is empty

        graphNode** link = &(repo->nodes[i]); // Pointer to the head pointer of commits list
        while (*link != NULL) {
            graphNode* current = *link;
            if (current == recentCommit) {
                *link = current->nextParent; // Bypass the deleted node
                found = 1;
            } else {
                ...
                while (*parentLink != NULL) {
                    if (*parentLink == recentCommit) {
                        *parentLink = recentCommit->nextParent; // Adjust parent link to bypass
                        found = 1;
                    }
                    parentLink = &((*parentLink)->nextParent);
                }
            }
            if (*link != NULL) link = &((*link)->nextParent); // Move to the next node in the list
        }
    }
    ...
    if (!found) {
        ...
        free(recentCommit->commit); // Free the commit data
        free(recentCommit); // Free the node
    }
}
    ...
    do {
        ...
        switch (choice) {
            ...
            case 3:
                ...
                graphNode* recentCommit = stack->top->node; // Get the top commit
                const char* originalFileName = recentCommit->commit->originalFileName;
                
                graphNode* recentCommit2 = stack->top->next->node; // Get the commit below the top one
                const char* updatedFileName = recentCommit2->commit->updatedFileName;
            ...
            case 5:
                ...
                repository* myRepo2 = copyRepository(myRepo);
                printRepository(myRepo2);
                break;
            case 6:
