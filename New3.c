...
void printRepository(repository* repo) {
    static int lastBranchCount = 0;

    printf("Repository Contents:\n");
    printf("Current Branch Index: %d\n", repo->currentBranchIndex);
    printf("Branch Count: %d\n", repo->branchCount-1);
    
    if (repo->branchCount > lastBranchCount) {
        printf("Branches have increased by %d.\n", repo->branchCount - lastBranchCount-1);
    }
    lastBranchCount = repo->branchCount;
    // Print branches
    printf("Branches:\n");
...
repository* copyBranch(repository* originalRepo) {
    char repoName[] = "myRepo2";
...
void deleteMostRecentCommit(repository *repo, commitStack *stack) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        while (*link != NULL) {
            ...
            if (*link != NULL)
                link = &((*link)->nextParent); // Move to the next node in the list
        }
    }
    ...
    do {
        ...
        switch (choice) {
            ...
            case 5:
                ...
                repository* myRepo2 = copyBranch(myRepo);
